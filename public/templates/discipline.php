...
<body>
  ...
  $buildAthleteUrl = function (array $runner): string {
    $athleteUrl = '/athlete/' . urlencode((string) ($runner['iofId'] ?? ''));
    $counted = $runner['countedEvents'] ?? ($runner['events'] ?? []);
    $eventIds = [];
    foreach ($counted as $ev) {
      $eventId = isset($ev['eventorId']) ? (string) $ev['eventorId'] : '';
      if ($eventId !== '') {
        $eventIds[$eventId] = $eventId;
      }
    }
    if (!empty($eventIds)) {
      $athleteUrl .= '?eventorIds=' . urlencode(implode(',', array_values($eventIds)));
    }
    return $athleteUrl;
  };
  ...
  foreach ($rankings as $r) {
    if ($groupFilter && (($r['group'] ?? null) !== $groupFilter))
      continue;
    $filtered[] = $r;
  }
    ...
    <section class="discipline-card edetabel-card">
      ...
      <?php else: ?>
        ...
        <div class="leader leader-big">
          ...
          <div class="leader-name"> <a href="<?php echo htmlspecialchars($buildAthleteUrl($leader)); ?>">
              <?php echo htmlspecialchars($leader['firstname'] . ' ' . $leader['lastname']); ?>
            </a></div>
          <div class="leader-info">
            <div class="leader-points sum"><?php echo htmlspecialchars((string) ($leader['totalPoints'] ?? 0)); ?> punkti
            </div>
          </div>
        </div>
        ...
        <div class="scoreboard-list top-list compact">
          <?php foreach ($filtered as $r):
            ...
            <div class="scoreboard-list-item">
              <div class="scoreboard-list-item__col-place"><?php echo htmlspecialchars((string) ($r['place'] ?? '-')); ?>.
              </div>
              <div class="scoreboard-list-item__col-name">
                <a class="r-link" href="<?php echo htmlspecialchars($buildAthleteUrl($r)); ?>">
              ...
              <div class="scoreboard-list-item__col-dob">
                <?php echo htmlspecialchars(substr($r['birthdate'] ?? '', 0, 4)); ?></div>
              <div class="scoreboard-list-item__col-club"><?php echo ($r['clubname']) ?></div>
              <div class="scoreboard-list-item__col-points"><span
                  class="sum"><?php echo (string) ($r['totalPoints'] ?? 0); ?></span>
              </div>
